  ...
  def chooseAction(self, gameState):
    ...
    if depth_act_dict == {}:
      ...
      if best_action == Directions.STOP :
        ...
        #check depth-1 for best action
        # if depth-1 is not in dict, then choose random action
        # if depth-1 is in dict, then choose that action
        if best_depth-1 in depth_act_dict.keys():
          best_action = depth_act_dict[best_depth-1]
        else:
