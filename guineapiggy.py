...
class FluidAgent(CaptureAgent):
  ...
  # evaluation function
  def evaluationFunction(self, gameState, mode):
    # heuristic function to evaluate the state of the game
  ...
  def max_agent(self, gameState, mode, agent_ID, depth, total_compute_time = math.inf, alpha = -math.inf, beta = math.inf):
    
    if depth == 0:
      return self.evaluationFunction(gameState, mode), None
    
    # hash (gameState, agent_ID, depth) and store (value, action) associated with this in a dictionary
    hash_val = hash((gameState, mode, agent_ID, depth))
    # check if this hash already exists in the dictionary
    if hash_val in self.max_agent_dict:
      return self.max_agent_dict[hash_val]
    ...
    successor_list = self.getSuccessors(gameState, agent_ID)
    # move ordering based on heuristic
    successor_list.sort(key = lambda x: self.evaluationFunction(x[0], mode), reverse = True)
    ...
    for successor, action in successor_list:
      ...
      if enemy_conf != None:
        ...
        current_time = time.time()
        time_left = total_compute_time - (current_time - start_time)
        act_value,_ = self.min_agent(successor, mode, enemy_ID, depth-1, time_left, alpha, beta)
        ...
        current_time = time.time()
        time_left = total_compute_time - (current_time - start_time)
        act_value,_ = self.max_agent(successor, mode, agent_ID, depth-1, time_left, alpha, beta)
  ...
  def min_agent(self, gameState, mode, agent_ID, depth, total_compute_time = math.inf, alpha = -math.inf, beta = math.inf):
    if depth == 0:
      return self.evaluationFunction(gameState, mode), None
    ...
    successor_list = self.getSuccessors(gameState, agent_ID)
    # move ordering based on heuristic
    successor_list.sort(key = lambda x: self.evaluationFunction(x[0], mode), reverse = False) # reverse = False for min agent because we want to minimize the heuristic value - explore the least heuristic value first
    ...
    for successor, action in successor_list:
        ...
        if enemy_conf != None:
          current_time = time.time()
          time_left = total_compute_time - (current_time - start_time)
          act_value,_ = self.max_agent(successor, mode, enemy_ID, depth-1, time_left, alpha, beta)
          ...
          current_time = time.time()
          time_left = total_compute_time - (current_time - start_time)
          act_value,_ = self.min_agent(successor, mode, agent_ID, depth-1, time_left, alpha, beta)
  ...
  def chooseAction(self, gameState):
    ...
    # compute a mode of action for the agent : offense or defense
    # mode = 'offense'
    ...
    if self.index == team_list[0]:
      ...
      teammate_index = team_list[0]

    # find my state: pac or ghost
    AmPac = gameState.getAgentState(self.index).isPacman
    teammate_pac = gameState.getAgentState(teammate_index).isPacman

    # find positions
    my_pos = gameState.getAgentPosition(self.index)
    teammate_pos = gameState.getAgentPosition(teammate_index)
    if teammate_pos == None:
      teammate_pos = gameState.getInitialAgentPosition(teammate_index)
    # print("my_pos = ", my_pos)
    # print("teammate_pos = ", teammate_pos)

    dist_to_start = self.getMazeDistance(my_pos, gameState.getInitialAgentPosition(self.index))
    teammate_dist_to_start = self.getMazeDistance(teammate_pos, gameState.getInitialAgentPosition(teammate_index))

    am_closer_to_center = False
    # check who is closer to the center line
    center_dist = self.center_dist_from_pos_dict[my_pos][0] 
    teammate_center_dist = self.center_dist_from_pos_dict[teammate_pos][0]
    
    at_same_pos = (my_pos == teammate_pos)
    ...
    if not AmPac and teammate_pac:
      # I am the only ghost, so I am in defense mode
      mode = 'defense'

    elif not AmPac and len(enemy_pac_pos) > 0:
      # if I am a ghost and enemy pac is visible to my team, then I am in defense mode
      mode = 'defense'

    elif not AmPac and dist_to_start < teammate_dist_to_start:
      # if I am a ghost and I am closer to my spawn point than my teammate, then I am in defense mode
      mode = 'defense'

    # what to do if I am Pac
    elif AmPac and center_dist < 3 and len(enemy_pac_pos) > 0:
      # I am a pacman, but close to the center line, and my teammate called for backup
      mode = 'defense' 

    # elif not AmPac and at_same_pos and self.index < teammate_index:
    #   # if I am a ghost and I am at the same position as my teammate and my index is smaller than my teammate's index, then I am in offense mode
    #   mode = 'offense'
    ...
    for depth in depth_list:
      ...
      v, best_action = self.max_agent(gameState, mode, self.index, depth, time_left)
      if best_action != None:
        depth_act_dict[depth] = best_action
      else:
        break
    ...
    # print("depth_act_dict = ", depth_act_dict)
    if depth_act_dict == {}:
      # print("Time taken by Fluid = ", time.time() - start_time)
      print("Fluid is Random")
      return random.choice(actions)
    else:
      # choose action with highest depth
      best_depth = max(depth_act_dict.keys())
